...
- An EKS cluster provisioned with a dedicated ScyllaDB node group with local NVMe SSDs.
  If you do not have one yet, follow [Set up an EKS cluster for ScyllaDB](https://operator.docs.scylladb.com/master/install-operator/provision-infrastructure/set-up-eks-cluster.md).
...
- The environment variables from the [EKS cluster setup guide](https://operator.docs.scylladb.com/master/install-operator/provision-infrastructure/set-up-eks-cluster.md#set-environment-variables) exported in your shell (at minimum `AWS_REGION`, `EKS_AZ_1`, `EKS_AZ_2`, `EKS_AZ_3`).
...
Follow [Configure nodes](https://operator.docs.scylladb.com/master/deploy-scylladb/before-you-deploy/configure-nodes.md), selecting the **EKS** tab where applicable, then return here to deploy the cluster.
...
To tear down the EKS infrastructure, follow the [Clean up](https://operator.docs.scylladb.com/master/install-operator/provision-infrastructure/set-up-eks-cluster.md#clean-up) section of the EKS cluster setup guide.
