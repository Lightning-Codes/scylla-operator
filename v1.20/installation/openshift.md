...
4. In the `Install Operator` dialog, configure your ScyllaDB Operator installation:
   ...
   * Select the `stable` Update Channel and version 1.20.2 from the list.
...
5. Create a `Subscription` object to install and subscribe to ScyllaDB Operator:
   ...
   spec:
     ...
     startingCSV: scylladb-operator.v1.20.2
     config:
       env:
       - name: ROLEARN
         value: "<role_arn>"
...
7. Wait for the ScyllaDB Operator to be installed:
   ...
   ```bash
   kubectl -n=scylla-operator wait --for=create --timeout=10m csv/scylladb-operator.v1.20.2 
   ```

   **Expected output**:
   ```console
   clusterserviceversion.operators.coreos.com/scylladb-operator.v1.20.2 condition met
   ```

   Wait for `ClusterServiceVersion` to reach `Succeeded` phase:
   ```bash
   kubectl wait -n=scylla-operator --timeout=5m --for=jsonpath='{.status.phase}'=Succeeded clusterserviceversions.operators.coreos.com/scylladb-operator.v1.20.2
   ```
